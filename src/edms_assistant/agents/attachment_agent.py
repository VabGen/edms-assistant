# src/edms_assistant/agents/attachment_agent.py
        ...
        try:
            ...
            # Если есть ID документа - получаем вложения и анализируем
                ...
                try:
                    ...
                    if "error" in doc_data:
                        return {
                            "messages": [
                                HumanMessage(content=user_message),
                                AIMessage(
                                    content=f"Ошибка получения документа: {doc_data['error']}"
                                ),
                            ],
                            "requires_execution": False,
                            "requires_clarification": False,
                    ...
                    async with DocumentClient(
                        service_token=state.service_token
                    ) as client:
                        attachments = await client.get_document_attachments(
                            state.document_id
                        )
                    ...
                    if attachments and len(attachments) > 0:
                        ...
                        if attachment_id:
                            ...
                            return {
                                "messages": [
                                    HumanMessage(content=user_message),
                                    AIMessage(
                                        content="Не найден ID вложения в документе"
                                    ),
                                ],
                                "requires_execution": False,
                                "requires_clarification": False,
                        ...
                        return {
                            "messages": [
                                HumanMessage(content=user_message),
                                AIMessage(
                                    content="В документе нет вложений для анализа"
                                ),
                            ],
                            "requires_execution": False,
                            "requires_clarification": False,
